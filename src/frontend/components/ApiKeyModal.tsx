      ...
      <main className="max-w-5xl mx-auto w-full p-3 sm:p-5 space-y-4 flex-1">
        ...
        {activeTab === 'keys' && (
          <div className="space-y-6">
            {!isPaidTier ? (
              <div className="bg-gradient-to-r from-red-950/90 via-black to-red-950/90 border border-red-600 rounded-3xl p-8 shadow-2xl space-y-4 text-center max-w-3xl mx-auto">
                <div className="w-16 h-16 rounded-2xl bg-red-950 border border-red-600 flex items-center justify-center mx-auto text-red-400 shadow-[0_0_30px_rgba(220,38,38,0.5)]">
                  <Lock size={32} />
                </div>
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-300 leading-relaxed max-w-xl mx-auto">
                    Developer API keys (<code className="text-red-400 font-bold">nvn_live_...</code>) are dedicated to subscribed accounts. Upgrade your account plan or purchase a developer API key suite to start sending requests.
                  </p>
                </div>
                <div className="pt-2">
                  <button
                    ...
                    className="px-8 py-3.5 bg-red-600 hover:bg-red-500 text-white font-bold text-xs rounded-xl transition-all shadow-xl inline-flex items-center gap-2.5 uppercase tracking-wider cursor-pointer"
                  >
                    <Crown size={18} />
              ...
              /* Generator Form */
              <form onSubmit={handleGenerateKey} className="bg-black/90 border border-red-900/80 rounded-2xl p-6 space-y-4 shadow-xl">
                <div className="flex items-center justify-between border-b border-red-950 pb-3">
                  ...
                  <span className="text-xs text-slate-400">Prefix format: <code className="text-red-400 font-bold">nvn_live_...</code></span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-1">
                  <input
                    ...
                    className="flex-1 bg-zinc-950 border border-red-900/80 rounded-xl px-4 py-3 text-slate-100 text-sm outline-none focus:border-red-500 transition-colors placeholder:text-zinc-600 font-mono"
                  />
                  <button
                    type="submit"
                    disabled={generating}
                    className="px-6 py-3 bg-red-600 hover:bg-red-500 text-white font-bold text-xs rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 shrink-0 uppercase tracking-wider"
                  >
                    <Plus size={18} />
                ...
                {newlyCreatedKey && (
                  <div className="p-4 bg-red-950/80 border border-red-600 rounded-xl space-y-2 animate-fadeIn mt-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-red-300 font-bold flex items-center gap-1.5">
                    ...
                    <code className="block p-3 bg-black rounded-lg border border-red-900/80 text-red-400 text-sm break-all font-mono selection:bg-red-900">
                      {newlyCreatedKey}
                    </code>
                  </div>
                )}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between px-1 border-b border-red-950 pb-2">
              ...
              {loading ? (
                ...
                <div className="p-12 text-center bg-black/60 border border-red-950 rounded-2xl space-y-3">
                  <Key size={32} className="mx-auto text-red-800" />
                  <p className="text-sm text-slate-300 font-bold">No API keys generated yet.</p>
                  <p className="text-xs text-slate-500">Create a key above to start making requests with <code className="text-red-400">nvn_live_...</code></p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-3">
                  {apiKeys.map((k) => {
                    ...
                    return (
                      <div
                        key={k.id}
                        className="bg-black/90 border border-red-950 hover:border-red-800/80 rounded-2xl p-5 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-lg"
                      >
                        <div className="space-y-1.5 overflow-hidden">
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-bold text-white">{k.name}</span>
                            <span className="text-[10px] bg-red-950 text-red-400 border border-red-900/80 px-2.5 py-0.5 rounded font-mono font-bold">
                              ACTIVE
                            </span>
                          </div>
                          <div className="flex items-center gap-3">
                            <code className="text-xs md:text-sm text-red-400 font-mono bg-zinc-950 px-3 py-1.5 rounded-xl border border-zinc-900 select-all">
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          <button
                            ...
                            className="px-4 py-2 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-slate-200 text-xs rounded-xl transition-colors flex items-center gap-2 cursor-pointer font-bold"
                          >
                          ...
                          <button
                            ...
                            className="p-2 text-slate-500 hover:text-red-400 hover:bg-red-950/80 rounded-xl transition-colors cursor-pointer"
        ...
        {activeTab === 'billing' && (
          <div className="space-y-8">
            ...
            <div className="p-6 md:p-8 bg-gradient-to-br from-red-950/90 via-black to-zinc-950 border border-red-800 rounded-3xl relative overflow-hidden shadow-2xl space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-red-900/80">
                <div className="flex items-center gap-4">
                  <div className="p-4 bg-black border border-red-600 text-red-500 rounded-2xl shadow-[0_0_20px_rgba(220,38,38,0.5)]">
                    <Key size={32} />
                  </div>
                  <div>
                    <div className="flex items-center gap-3 flex-wrap">
                      <h2 className="text-xl md:text-2xl font-black text-red-500 uppercase tracking-widest">
                        1. DEDICATED DEVELOPER API KEY SUITE
                      </h2>
                      <span className="text-[10px] bg-red-950 text-red-400 border border-red-800 px-3 py-1 rounded-full font-bold uppercase tracking-widest">
                ...
                <div className="flex items-center gap-2 bg-black/90 border border-red-950 p-1.5 rounded-2xl shrink-0">
                  <button
                    ...
                    className={clsx(
                      ...
                      apiBillingCycle === 'monthly' ? "bg-red-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
                    )}
                  >
                    Monthly Billing
                  </button>
                  <button
                    ...
                    className={clsx(
                      ...
                      apiBillingCycle === 'yearly' ? "bg-emerald-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
                    )}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
                <div className="p-6 bg-black/90 border border-red-950 rounded-2xl space-y-2">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">DEVELOPER ACCESS RATE</span>
                  ...
                  {apiBillingCycle === 'yearly' && (
                    <span className="text-xs text-emerald-400 font-mono font-bold flex items-center gap-1 pt-1">
                      <Tag size={13} />
                ...
                <div className="p-6 bg-black/90 border border-red-950 rounded-2xl space-y-2">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">INCLUDED CAPABILITIES</span>
                  <ul className="text-xs text-slate-200 space-y-2 font-mono">
            ...
            {/* Account Plan Banner */}
            <div className="p-6 md:p-8 bg-gradient-to-r from-red-950/90 via-black to-zinc-950 border border-red-800 rounded-3xl space-y-6 shadow-2xl">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-red-950">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-red-600 hover:bg-red-500 text-white font-bold text-xs rounded-xl transition-all shadow-lg flex items-center gap-2 cursor-pointer uppercase tracking-wider shrink-0"
                >
                  <Crown size={16} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="p-5 bg-black/90 border border-zinc-800 rounded-2xl space-y-3">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest">FREE PLAN</div>
                  <div className="text-2xl font-extrabold text-white">₦0 <span className="text-xs text-slate-500 font-normal">/mo</span></div>
                  <ul className="text-xs text-slate-400 space-y-2 pt-3 border-t border-zinc-900 font-mono">
                ...
                <div className="p-5 bg-black/90 border border-blue-900/80 rounded-2xl space-y-3">
                  ...
                  <div className="text-2xl font-extrabold text-blue-400">
                    ...
                    <span className="text-xs text-slate-500 font-normal">{apiBillingCycle === 'yearly' ? '/yr' : '/mo'}</span>
                  </div>
                  <ul className="text-xs text-slate-300 space-y-2 pt-3 border-t border-zinc-900 font-mono">
            ...
            <div className="p-5 bg-zinc-950 border border-zinc-800 rounded-2xl space-y-2">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Need higher token limits or custom API quotas for your business? Message the admin on Telegram (<a href="https://t.me/nova_tech_1" target="_blank" rel="noreferrer" className="text-red-400 hover:underline">@nova_tech_1</a>) or request a tier grant through the Support Desk.
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'code' && (
          <div className="space-y-6">
            <div className="p-6 bg-black/90 border border-red-950 rounded-2xl space-y-3 shadow-lg">
              <h3 className="text-sm font-bold text-red-400 uppercase tracking-wider flex items-center gap-2">
                <Terminal size={18} />
                <span>OpenAI-Compatible REST API Endpoint</span>
              </h3>
              <div className="flex items-center gap-3 bg-zinc-950 p-3.5 rounded-xl border border-zinc-900">
                <code className="text-xs md:text-sm text-red-400 font-mono flex-1 select-all break-all font-bold">
                  {appUrl}/v1/chat/completions
                </code>
                <button
                  onClick={() => handleCopy(`${appUrl}/v1/chat/completions`, 'base-url')}
                  className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white text-xs font-bold rounded-xl transition-colors cursor-pointer shrink-0 uppercase tracking-wider"
                >
            ...
            {/* Python Example */}
            <div className="bg-black/90 border border-zinc-800 rounded-2xl p-6 space-y-3 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold text-amber-400 flex items-center gap-2 font-mono">
...
print(response.choices[0].message.content)`, 'python-code')}
                  className="text-xs text-slate-300 hover:text-white flex items-center gap-1.5 cursor-pointer font-bold bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-800"
                >
                  <Copy size={14} />
                  <span>Copy Python Code</span>
                </button>
              </div>
              <pre className="p-4 bg-zinc-950 rounded-xl text-slate-200 text-xs overflow-x-auto font-mono leading-relaxed border border-zinc-900 select-all">
{`import openai
...
console.log(completion.choices[0].message.content);`}
            ...
            <div className="bg-black/90 border border-zinc-800 rounded-2xl p-6 space-y-3 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold text-sky-400 flex items-center gap-2 font-mono">
