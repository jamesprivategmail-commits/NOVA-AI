...
import { Plus, MessageSquare, Trash2, Edit2, Settings, LogOut, Shield, Crown, Mail, Sparkles } from 'lucide-react';
...
export function Sidebar({
  ...
  return (
    <div className={twMerge(
      clsx(
        "flex flex-col h-full bg-[#171717] transition-all duration-300 w-[260px] shrink-0 relative border-r border-zinc-800/80",
      ...
      <div className="p-3 space-y-2">
        ...
        <button
          ...
          className="w-full flex items-center gap-2.5 bg-gradient-to-r from-red-950/60 to-zinc-900 border border-red-900/40 hover:border-red-600 text-red-200 py-2.5 px-3 rounded-xl transition-all font-semibold text-xs shadow-sm group"
        >
          <div className="p-1 bg-red-600/30 rounded-lg text-red-400 group-hover:text-red-300">
            <Mail size={15} />
          </div>
          <span className="truncate">Email Campaign Tool</span>
        </button>
      </div>

      <div className="flex-1 overflow-y-auto px-3 space-y-0.5 mt-2">
        <div className="text-xs font-semibold text-zinc-500 px-3 py-2 mb-1">Recent Chats</div>
        {chats.map(chat => (
          <div
            key={chat.id}
            className={clsx(
      ...
      <div className="p-3 mt-auto space-y-1">
        {isAdmin && (
          <button
            onClick={onOpenAdmin}
            className="w-full flex items-center gap-3 p-2.5 rounded-lg text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-colors"
          >
            <Shield size={18} />
        ...
        <button
          ...
          className="w-full flex items-center gap-3 p-2.5 rounded-lg text-amber-400 hover:bg-amber-500/10 hover:text-amber-300 transition-colors"
        >
          <Crown size={18} />
        ...
        <button
          ...
          className="w-full flex items-center gap-3 p-2.5 rounded-lg text-zinc-400 hover:bg-zinc-800 hover:text-zinc-300 transition-colors"
        >
          <Settings size={18} />
